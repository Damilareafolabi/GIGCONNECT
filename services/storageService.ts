...
const seedData = () => {
    ...
    if (!get('users')) {
        ...
        const jobs: Job[] = [
            { id: 'job-1', employerId: 'employer-1', title: 'Senior React Developer', description: 'Build our next-gen platform using React and TypeScript.', category: 'Web Development', payment: 5000, deadline: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString(), status: JobStatus.Open, createdAt: new Date().toISOString(), isFeatured: true, paymentStatus: 'Unpaid' },
            { id: 'job-2', employerId: 'employer-1', title: 'UI/UX Designer', description: 'Design beautiful and intuitive user interfaces.', category: 'Graphic Design', payment: 3500, deadline: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000).toISOString(), status: JobStatus.Open, createdAt: new Date().toISOString(), isFeatured: false, paymentStatus: 'Unpaid' },
            { id: 'job-3', employerId: 'employer-1', title: 'Backend Node.js Engineer', description: 'Develop and maintain our server-side logic.', category: 'Web Development', payment: 4500, deadline: new Date(Date.now() + 20 * 24 * 60 * 60 * 1000).toISOString(), status: JobStatus.PendingApproval, createdAt: new Date().toISOString(), isFeatured: false, paymentStatus: 'Unpaid' },
        ];
        set<Job[]>('jobs', jobs);
